...
// Ensure Info is imported if not already. We will add it to the 'lucide-react' imports.
if (content.includes("from 'lucide-react';") && !content.includes("Info,")) {
    content = content.replace("from 'lucide-react';", "Info,\n} from 'lucide-react';");
}
...
const replacement = `const InfoTooltip: React.FC<{ text: string }> = ({ text }) => {
  ...
  return (
    <div className="relative inline-block" ref={tooltipRef}>
      <button
        ...
        className="text-[#8A98A8] dark:text-[#707070] text-[11px] p-1 flex items-center justify-center outline-none hover:text-[#2898D8] transition-colors"
      >
        <Info size={12} />
      </button>
      <AnimatePresence>
        {open && (
           ...
           className="absolute top-full right-0 mt-1.5 bg-[#1A2332] text-white text-[11px] font-medium leading-[1.3] rounded-xl p-3 w-[180px] z-50 shadow-xl border border-[#3A3A3A] px-3.5 py-3"
         >
           {text}
         </motion.div>
        )}
...
const TodayScheduleList: React.FC<{ appointments: Appointment[], date: string }> = ({ appointments, date }) => {
  ...
  return (
    <div className="bg-[#FFFFFF] dark:bg-[#242424] p-5 rounded-[2rem] shadow-[0_1px_4px_rgba(0,0,0,0.06)] border border-[#D0D8E4] dark:border-[#3A3A3A]">
      <h4 className="text-[10px] font-bold uppercase tracking-widest text-[#8A98A8] mb-4 text-center">Sua agenda de hoje</h4>
      <div className="space-y-3">
        {todayApts.length > 0 ? (
          todayApts.map((apt, idx) => (
            <div key={idx} className="flex items-center gap-3 p-3 rounded-2xl bg-[#F4F7FB] dark:bg-[#1A1A1A] border border-[#E8EEF5] dark:border-[#2F2F2F]">
              <div className="text-[11px] font-mono font-bold text-[#5A6878] dark:text-[#8A98A8] shrink-0 w-10 text-center">
                {apt.time}
              </div>
              <div className="flex-1 min-w-0">
                <p className="text-sm font-bold text-[#1A2332] dark:text-[#F8F8F8] truncate leading-tight">{apt.clientName}</p>
                <p className="text-[10px] font-medium text-[#8A98A8] truncate mt-0.5">{apt.service}</p>
              </div>
              <div className="shrink-0 flex items-center">
                {apt.status === 'completed' && <span className="bg-green-500/10 text-green-500 text-[9px] font-bold px-2 py-1 rounded-full flex items-center gap-1"><span className="w-1.5 h-1.5 rounded-full bg-green-500"></span> Concluído</span>}
                {apt.status === 'scheduled' && <span className="bg-blue-500/10 text-[#2898D8] text-[9px] font-bold px-2 py-1 rounded-full flex items-center gap-1"><span className="w-1.5 h-1.5 rounded-full bg-[#2898D8]"></span> Agendado</span>}
                {apt.status === 'no-show' && <span className="bg-red-500/10 text-red-500 text-[9px] font-bold px-2 py-1 rounded-full flex items-center gap-1"><span className="w-1.5 h-1.5 rounded-full bg-red-500"></span> Falta</span>}
                {apt.status === 'canceled' && <span className="bg-gray-500/10 text-gray-500 text-[9px] font-bold px-2 py-1 rounded-full flex items-center gap-1"><span className="w-1.5 h-1.5 rounded-full bg-gray-500"></span> Canc</span>}
              </div>
            </div>
          ))
        ) : (
...
const ReportsView: React.FC = () => {
  ...
  return (
    <div className="space-y-6 pb-24 max-w-full overflow-x-hidden">
      {/* 4 chips sempre visíveis em grid */}
      <div className="sticky top-0 bg-[#F4F7FB] dark:bg-[#1A1A1A] z-10 pt-2 pb-3">
          ...
          {(['dia', 'semana', 'mes', 'ano'] as const).map(p => {
             ...
             return (
               <button
                 ...
                 className={\`flex items-center justify-center gap-1 py-2 px-1 rounded-full text-xs font-semibold transition-all w-full \${
        ...
        {/* Mostra o período ativo caso não seja o atual/hoje */}
        {!isCurrentDateSelected && (
           <div className="mt-2 text-center text-xs font-bold text-[#2898D8]">
             {formatPeriodDisplay()}
           </div>
        )}
      </div>
      ...
      {/* 3 chips compactos */}
      <div className="grid grid-cols-3 gap-2">
         <div className="bg-[#FFFFFF] dark:bg-[#242424] p-3 py-4 rounded-2xl shadow-[0_1px_4px_rgba(0,0,0,0.06)] border border-[#E8EEF5] dark:border-[#3A3A3A] text-center flex flex-col justify-center gap-1.5 relative">
            <div className="absolute top-2 right-2"><InfoTooltip text="Valor médio cobrado por atendimento no período" /></div>
            <p className="text-[9px] font-bold uppercase tracking-widest text-[#8A98A8] leading-tight flex justify-center items-center h-6">Ticket médio</p>
            <p className="text-sm font-black text-[#1A2332] dark:text-[#F8F8F8]">{formatCurrency(stats.current.ticket)}</p>
         </div>
         <div className="bg-[#FFFFFF] dark:bg-[#242424] p-3 py-4 rounded-2xl shadow-[0_1px_4px_rgba(0,0,0,0.06)] border border-[#E8EEF5] dark:border-[#3A3A3A] text-center flex flex-col justify-center gap-1.5 relative">
            <div className="absolute top-2 right-2"><InfoTooltip text="Clientes que vieram pela primeira vez no período" /></div>
            <p className="text-[9px] font-bold uppercase tracking-widest text-[#8A98A8] leading-tight flex justify-center items-center h-6 px-4">Novos clientes</p>
            <p className="text-sm font-black text-[#1A2332] dark:text-[#F8F8F8]">{stats.newClients}</p>
         </div>
         <div className="bg-[#FFFFFF] dark:bg-[#242424] p-3 py-4 rounded-2xl shadow-[0_1px_4px_rgba(0,0,0,0.06)] border border-[#E8EEF5] dark:border-[#3A3A3A] text-center flex flex-col justify-center gap-1.5 relative">
            <div className="absolute top-2 right-2"><InfoTooltip text="Clientes que agendaram e não compareceram" /></div>
            <p className="text-[9px] font-bold uppercase tracking-widest text-[#8A98A8] leading-tight flex justify-center items-center h-6">Faltas</p>
      ...
      {/* Clientes de Hoje / Top 5 Clientes */}
      {period === 'dia' ? (
        <div className="bg-[#FFFFFF] dark:bg-[#242424] p-5 rounded-[2rem] shadow-[0_1px_4px_rgba(0,0,0,0.06)] border border-[#D0D8E4] dark:border-[#3A3A3A]">
          <h4 className="text-[10px] font-bold uppercase tracking-widest text-[#8A98A8] mb-4">Clientes de Hoje</h4>
          <div className="space-y-4">
            {stats.current.apts.length > 0 ? stats.current.apts.map((apt, idx) => (
              <div key={idx} className="flex items-center gap-3">
                <div className={\`w-10 h-10 rounded-full flex items-center justify-center text-white font-bold text-xs shrink-0 \${getAvatarColor(apt.clientName)}\`}>
                  {getInitials(apt.clientName)}
                </div>
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-bold text-[#1A2332] dark:text-[#F8F8F8] truncate leading-tight">{apt.clientName}</p>
                  <div className="flex items-center gap-2 mt-0.5">
                    <p className="text-[10px] font-medium text-[#8A98A8]">{apt.service}</p>
                    <span className="w-1 h-1 bg-[#D0D8E4] dark:bg-[#3A3A3A] rounded-full"></span>
                    <p className="text-[10px] font-bold text-[#5A6878] dark:text-[#8A98A8]">{apt.time}</p>
                  </div>
                </div>
                <div className="shrink-0">
                   {apt.status === 'completed' && <span className="text-green-500 text-[14px]">✅</span>}
                   {apt.status === 'scheduled' && <span className="text-[#2898D8] text-[14px]">⏳</span>}
                   {apt.status === 'no-show' && <span className="text-red-500 text-[14px]">❌</span>}
                   {apt.status === 'canceled' && <span className="text-gray-500 text-[14px]">🚫</span>}
                </div>
              </div>
            )) : (
              <p className="text-center text-[#8A98A8] text-xs py-4 font-medium">Nenhum cliente agendado hoje</p>
            )}
          </div>
        </div>
      ) : (
...
console.log('Successfully replaced ReportsView.');
